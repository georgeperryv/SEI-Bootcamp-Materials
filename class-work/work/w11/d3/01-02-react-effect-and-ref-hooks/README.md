...
> IMPORTANT:  If you use a different SECRET in the .env for **any** MERN-Stack project, you'll need to manually delete any token that might exist using devtools.  This is because all of our projects' tokens are being stored for localhost:3000 and they won't be compatible if the SECRET used to sign the tokens is different.
